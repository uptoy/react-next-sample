...
test('Cart should display correct total', async () => {
  const state = getStateWithItems({ testItem: 3 }, { testItem: { name: 'Test Product', price: 11.11 } as Product });
...
test('cannot checkout with an empty cart', async () => {
  ...
  expect(table).not.toHaveStyle({ 'border-color': 'rgb(111, 103, 91)' });
  userEvent.click(checkout);
  await screen.findByText('Cart must not be empty', { selector: '.errorBox' });
  expect(table).toHaveStyle({ 'border-color': 'gray' });
});
