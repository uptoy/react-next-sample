...
describe('Cart Reducer', () => {
  ...
  describe('extra reducers', () => {
    ...
    it('should respond to fulfilled checkout action with success', () => {
      ...
      const state = cartReducer(initialState, action);
      expect(state.checkoutState).toEqual('SUCCESS');
      expect(state.items).toEqual({});
    });
...
describe('thunks', () => {
  describe('checkoutAction()', () => {
    ...
    it('should dispatch two actions on success', async () => {
      ...
      await thunk(dispatch, () => state);
      expect(dispatch.mock.calls.map(call => call[0].type)).toEqual(['cart/checkout/pending', 'cart/checkout/fulfilled']);
    });
  });
  ...
  describe('checkout()', () => {
    it('should dispatch two actions on failure', async () => {
      ...
      await thunk(dispatch, () => state, {});
      expect(dispatch.mock.calls.map(call => call[0].type)).toEqual(['cart/checkout/pending', 'cart/checkout/rejected']);
    });
